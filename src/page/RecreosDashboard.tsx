...
// ============================================================================
// COMPONENTE PRINCIPAL DEL DASHBOARD - RECREOS
// ============================================================================
...
const RecreosDashboard = () => {
  ...
  // ============================================================================
  // FILTRADO ESPECÍFICO PARA RECREOS
  // ============================================================================
  // Filtrar datos específicamente para Recreos
  const recreosRows = useMemo(() => {
    if (!F.proyectoEstrategico) return rows;
    
    return rows.filter(row => {
      const proyectoRow = String(row[F.proyectoEstrategico] ?? '').toLowerCase();
      return proyectoRow.includes('recreos') || 
             proyectoRow.includes('recreo') ||
             proyectoRow.includes('espacios de recreación') ||
             proyectoRow.includes('actividades recreativas');
    });
  }, [rows]);
  ...
  const opciones = useMemo(() => getFilterOptions(recreosRows, filters), [recreosRows, filters]);
  const combinedFilters = useMemo(() => combineDateFields(filters), [filters]);
  const filtered = useMemo(() => applyFilters(recreosRows, combinedFilters), [recreosRows, combinedFilters]);
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-content">
        ...
        {status === 'Cargando...' && (
          <div className="loading-container">
            ...
            <div className="loading-text">
              <h3>Cargando datos de Recreos...</h3>
              <p>Por favor espera mientras se procesan los espacios recreativos</p>
            </div>
          </div>
        )}
        ...
        <div className="kpis-section">
          <div className="kpis-container">
            ...
            <div className="kpis-grid kpis-row-5">
              <Kpi 
                label="Total espacios recreativos" 
                value={k.totalObras} 
                trend="neutral"
              />
              <Kpi 
                label="Inversión recreación" 
                value={k.invTotal} 
                format="money" 
                abbreviate 
                digits={1}
              ...
              />
              <Kpi 
                label="Recreos entregados" 
                value={k.entregadas} 
                subtitle={`${Math.round(k.pctEntregadas * 100)}% del total`}
                trend="up"
              />
              <Kpi 
                label="Alertas recreativas" 
                value={k.alertas}
        ...
        <div className="map-main-panel">
          ...
          <div className="map-legend">
            <h4>Leyenda por Dependencia - Recreos:</h4>
        ...
        <div className="content-section" style={{ display: 'block' }}>
          ...
          {comboDataset.length > 0 && (
            <div className="chart-card">
              <ComboBars
                title="Inversión vs Presupuesto Ejecutado - Recreos"
          ...
          <div className="tables-grid">
            {/* Tabla de recreos entregados */}
            <div className="table-card">
              <WorksTable
                title="Espacios Recreativos Entregados"
                works={entregadas}
                type="entregadas"
                maxRows={6}
              />
            ...
            {/* Tabla de recreos por entregar */}
            <div className="table-card">
              <WorksTable
                title="Espacios Recreativos por Entregar"
                works={porEntregar}
                type="porEntregar"
                maxRows={4}
              />
            ...
            {/* Tabla de alertas y riesgos recreativos */}
            <div className="table-card">
              <AlertsTable
                alerts={alertas}
                maxRows={6}
      ...
      {/* ========================================================================
           ...
           (Reutilizando los mismos estilos del dashboard general)
       ======================================================================== */}
      <style>{`
...
export default RecreosDashboard;
